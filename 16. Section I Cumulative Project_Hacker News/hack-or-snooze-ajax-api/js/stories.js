...
function generateStoryMarkup(story, deleteButton = false) {
  ...
  const seeFavoriteNot = Boolean(currentUser);
  return $(`
      <li id="${story.storyId}">
        ${deleteButton ? removeHTMLbtn() : ''}
        ${seeFavoriteNot ? favoriteNotStoryHTML(story, currentUser) : ''}
...
function favoriteNotStoryHTML(story, user){
  const star = user.userFavorite(story) ? 'fas' : 'far';
  return `
      <span class="star">
