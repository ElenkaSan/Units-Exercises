...
class StoryList {
   ...
   */
   async addStory(user, {title, author, url}){
    ...
    this.stories.unsfift(story);
    user.ownStories.unsfift(story);
    return story;
  }
  ...
  // Allow logged in users to remove a story. Once a story has been deleted, 
...
class User {
  ...
  // The methods for adding and removing favorite status on a story should be 
  // defined in the User class.
  async addingFavorite(story) {
    this.favorites.push(story);
    await this._changeFavorite('add', story)
  }

  async unFavorite(story) {
    this.favorites = this.favorites.filter(s => s.storyId !== story.storyId);
    await this._changeFavorite('remove', story);
  }

  async _changeFavorite(change, story) {
